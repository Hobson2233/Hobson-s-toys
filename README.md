...
当前版本 0.3.3，从 [Releases](../../releases) 下载。
...
账号与配置保存在 `C:\ProgramData\CampusLogin\`，不随程序文件移动。程序自身升级时的临时文件（下载中的包、旧版本备份）也放在这个目录，**不会写到你放 exe 的地方** —— 升级完桌面（或你放 exe 的任意目录）里仍然只有 exe 本身。开机自启由系统「任务计划程序」里名为 `CampusLogin` 的任务实现（登录时触发），任务里记录程序的绝对路径，移动程序后该功能失效，取消并重新勾选即可恢复；从旧版本升级上来时界面会提示「旧设置」，同样取消再勾一次，新的守护功能才会生效。
