...
**当前版本 `0.1.0`** —— 还在早期，功能和配置格式都可能变。下载见 [Releases](../../releases)。
...
> Release 页面上显示为「校园网自动登录」，下载下来的文件名是 `campus-login.exe`
> （GitHub 的 Release 资产不接受非 ASCII 文件名，所以用英文名 + 中文显示名）。
> 如果还没有 Release，就按下一节从源码自己构建一个。
