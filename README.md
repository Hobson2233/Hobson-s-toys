...
当前版本 0.3.5，从 [Releases](../../releases) 下载。
...
- 使用说明内置于程序，无需另找文档
- 设置页可一键「清除本机保存的账号密码」，连同日志一起删掉，不用手翻系统目录
- 程序内不含账号密码，可直接分发给他人
...
- **不需要管理员权限**：账号数据写在 `C:\ProgramData\CampusLogin\`（该目录不可写时自动退回用户目录），
  开机自启用的是当前用户的登录计划任务，全程不要求提权，不会弹 UAC
- 磁盘占用约 12 MB
...
账号与配置保存在 `C:\ProgramData\CampusLogin\`，不随程序文件移动。程序自身升级时的临时文件（下载中的包、旧版本备份）也放在这个目录，**不会写到你放 exe 的地方** —— 升级完桌面（或你放 exe 的任意目录）里仍然只有 exe 本身。旧版本备份会在下次启动时自动删掉；万一那会儿删不掉（比如程序正开着），它过一会儿会自己再试几次。开机自启由系统「任务计划程序」里名为 `CampusLogin` 的任务实现（登录时触发），任务里记录程序的绝对路径，移动程序后该功能失效，取消并重新勾选即可恢复；从旧版本升级上来时界面会提示「旧设置」，同样取消再勾一次，新的守护功能才会生效。
...
**账号密码不会上传到任何服务器。** 这句话具体到可以自己核对的程度：
...
- 设置页最下面的「清除本机保存的账号密码」；
- 命令行 `校园网自动登录.exe --purge`（退出码 0 = 清干净了）。

两者做的是同一件事：删掉 `config.json`、`accounts.json`、`last-result.json` 和 `campus-login.log`，**不可恢复**。数据目录本身会留下（里面已经空了，下次运行照常重建）。清除后需要重新输入账号密码；如果开机自启还开着，建议一并取消 —— 否则开机时会跑一次但登不上。
