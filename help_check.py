...
# 更新来源域名同理（2026-09-21 加）：HELP_TEXT 里必须**写死字面量**才搜得到
# （原因见下面 30 分钟那条的说明），所以这里拿 updater 的 BASE_URL_HINT
# 反算出域名来比对 —— 哪天换了发布域名却忘了改说明，这条会立刻报 BAD。
try:
    import updater as _U
    _SRC_HOST = _U.BASE_URL_HINT.split("//", 1)[-1]
except Exception:                                    # 源码不在旁边就别拦
    _SRC_HOST = "hobson2233.dpdns.org"
...
CASES = [
    ...
    # 清除本机凭据的界面入口（2026-09-21 加）。以前只有命令行 --purge，说明里
    # 也只教命令行 —— 现在设置页最下面有个链接了，文案必须跟着改，否则用户
    # 照着说明去找命令行，永远发现不了那个链接。
    ("清除本机保存的账号密码", True, "0.3.5 清除凭据的界面入口"),
    # 更新来源（2026-09-21 加）。域名必须写死才搜得到（同上面 30 分钟那条），
    # 所以这里拿 updater 反算出来的域名比对，防「换了域名忘改说明」。
    (_SRC_HOST, True, "更新来源域名（应与 updater.MANIFEST_URL 一致）"),
